...
def clusterKMeansBase(corr0, maxNumClusters=10, n_init=10, debug=False):
    ...
    for init in range(0, n_init):
        for num_clusters in range(2, maxNumClusters+1):
            kmeans_ = KMeans(n_clusters=(maxNumClusters+2-num_clusters), n_init=init+1, random_state=3425) #n_jobs=None, n_init=1) #n_jobs=None - use all CPUs
...
def clusterKMeansTop(corr0: pd.DataFrame, maxNumClusters=None, n_init=10):
    ...
    corr1, clstrs, silh = clusterKMeansBase(corr0, maxNumClusters=min(maxNumClusters, corr0.shape[1]-1), n_init=1)#n_init)
    #print("clstrs keys:"+str(clstrs.keys()))
    #for i in clstrs.keys():
