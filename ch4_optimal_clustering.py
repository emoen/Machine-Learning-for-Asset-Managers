...
def clusterKMeansBase(corr0, maxNumClusters=10, n_init=10, debug=False):
    ...
    for init in range(0, n_init):
        for num_clusters in range(4, maxNumClusters+1):
            #(maxNumClusters + 2 - num_clusters), n_init=init+1
            #print("(num_clstrs, init):"+str(num_clusters), end=" ")
            #print(init+1)
            kmeans_ = KMeans(n_clusters=num_clusters, n_init=10, random_state=3425) #n_jobs=None, n_init=1) #n_jobs=None - use all CPUs
